...
Knowledge modules are stored in the repository's `knowledge/` directory and are selected by technical-art problem category. The repository currently includes a populated `texture-maps.md` module covering texture maps as data carriers, swizzling, PBR map semantics, color space, roughness, metallic, AO, and tangent-space normal maps.
